...
public final class JavacSourceFile extends Command {
    private static final String COMMAND = ":javac";
    private final Evaluator evaluator;
    private final ConsoleLogger logger;

    public JavacSourceFile(Evaluator evaluator, ConsoleLogger logger) {
        super(COMMAND + " <path> - compile and load source file ", startsWith(COMMAND), new CommandCompleter(COMMAND));
    ...
    public void execute(String expression) {
        ...
        if (!path.isEmpty()) {
            try {
                logger.info(evaluator.parseExpression(Strings.lines(path.map(asFile()).get()).toString("\n")).right().toString());
                // System.out.println(path.map(asFile()).get());
                // System.out.println(path.map(asFile()).get().getName());
